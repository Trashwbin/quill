...
> 前两篇我们聊了：CLI vs MCP 争的是管道问题，真正缺的是水龙头（[第一篇](/posts/cli-vs-mcp-vs-skills/)）；Agent 帮不了你是因为三层壁垒（[第二篇](/posts/agent-bottleneck-data-sovereignty/)）。这篇看看现实中大家在怎么应对这面墙。
...
豆包手机最典型——你不给我 API？行，我直接读你的屏幕，替用户点。

**本质还是爬虫。** 从爬网页变成了爬屏幕。技术更酷了，但脆弱性和灰色地带一点没少。
