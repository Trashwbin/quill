...
值得注意的是，优化层本身也在被优化。比如 Oh My ClaudeCode（OMC）[^11] 三个月内从零涨到 14,500 星，它的做法就是把 ECC 的配置注入和 OMO 的多 agent 编排合在一起，再用 Claude Code 的 plugin 系统做原生集成。这说明优化层的迭代速度很快，而且不同思路之间正在互相吸收。
