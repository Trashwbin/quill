...
graph TB
    ...
    subgraph "第二层：组织管控"
        W2["飞书、Jira、Google Workspace<br/>平台有 API，但企业 IT 不批准<br/>你看得到数据，但程序读不到"]
    end
    
    W1 --> W2
    
    W2 --> After["数据到手之后<br/>格式不统一？LLM 能处理<br/>↑ 这不是壁垒，是工程摩擦"]
...
微信不会做 `wx auth login`，淘宝不会开放比价 API，抖音不会给你推荐数据。[上一篇](/posts/cli-vs-mcp-vs-skills/)从技术角度论证了 CLI 完全能实现 OAuth（`gh auth login` 就是先例），所以这不是技术障碍[^1]。
