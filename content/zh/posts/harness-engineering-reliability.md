...
但这里还有一个经常被忽略的事实：**就算你走的是正规接口，Agent 依然可能把事情做坏。**

从今天的实践看，Agent 落地时最先撞上的，通常是两类连续出现的问题：
...
最近，开始有一些作者和团队把第二类问题集中放进：**Harness Engineering** 这个名字下面。

## Harness 是什么

LangChain 在 2026 年 3 月给了一个很干净的定义：
...
**Harness 是模型之外、但仍在构建者可工程化控制范围内、让 Agent 真正能干活的那部分系统。**

模型只负责“生成下一个 token”。但一个能工作的 Agent 还需要别的东西：
...
更准确的理解是：**Prompt、Context、Harness 不是互相替代，而是一层层向外包裹。**
...
- **Context Engineering** 解决的是：哪些信息该被放进窗口里
...
Anthropic 在 2025 年 11 月谈“long-running agents”时，已经把 Claude Agent SDK 称为“一种通用的 agent harness”，并且系统讨论了 compaction、initializer agent、progress file、feature list、git commit、end-to-end testing 这些做法[^2]。
...
换句话说，Harness Engineering 当然包含可靠性建设，但它并不等于可靠性工程。更完整地说，它至少同时覆盖四件事：
...
- **Harness**：AGENTS.md、bash 工具、sandbox、测试命令、progress file 这些现成部件
...
Anthropic 那篇文章给了一个很典型的例子。对于跨多个上下文窗口持续工作的 coding agent，他们发现单靠 compaction 不够，Agent 常见的失败方式包括：试图一次性做太多事、把项目做成半拉子工程、后来者读不懂前人的状态、以及在没充分验证的情况下就宣布任务完成[^2]。

于是他们没有去等“更强的模型”，而是去改环境：

- 第一轮由 initializer agent 先搭环境
...
如果要把这个判断再往前推一步，我觉得 Chad Fowler 在 2026 年初提出的一个说法很有启发：这不仅仅是“新封装”，更像是一次 **rigor 的迁移**[^5]。

他的意思不是工程纪律消失了，而是工程纪律换了位置。过去，严格性更多体现在“人如何逐行写代码”；现在，严格性越来越体现在“系统如何约束 agent、如何暴露失败、如何验证结果”。

换句话说，Agent 时代并没有降低对工程的要求，反而把要求从“手工实现”转移到了“环境设计、反馈循环、边界约束和结果评估”上。**以前的 rigor 更靠人维持，现在的 rigor 越来越要靠系统来承载。**
...
这也是为什么我更愿意把 Harness Engineering 理解成：**旧实践的重新封装，加上工程 rigor 的重新安置。** 名字不是重点，**可复用的工程边界**和**更靠近真实结果的纪律位置**才是重点。

## 为什么它现在突然重要了

因为 Agent 已经开始触碰一类过去没有被如此系统化讨论的问题：**长时间、跨上下文、带工具、带状态、需要自我验证的自主执行。**

而且，它已经不再只是一个讨论概念。最近几篇一手工程文章里，Harness 已经开始长出非常具体的技术形态。
...
其次，Anthropic 在 2026 年 3 月的新文章里，把 harness 从“单 agent + 验证回环”进一步推进到了 `planner / generator / evaluator` 的三 agent 结构[^7]。更重要的是，他们给出了很硬的对比：同一个任务里，solo 运行约 20 分钟、成本约 9 美元；完整 harness 运行接近 6 小时、成本约 200 美元，但产出质量有明显提升[^7]。这说明 harness 不只是“加一点 guardrail”，而是在重写 agent 的工作组织方式。

再次，LangChain 也给出了一个很难忽视的量化例子：在 Terminal Bench 2.0 上，同一个 `gpt-5.2-codex` 模型，只调整 harness，不换模型，分数就从 `52.8%` 提升到了 `66.5%`，整整提高 `13.7` 个点[^8]。换句话说，到了这个阶段，harness 已经不只是“让系统更整洁”，而是会直接改变 agent 的可测性能。
...
这篇真正想讨论的，不是 Agent 能不能接入真实世界，而是它在接入之后，能不能以一种受控、持续、可验证的方式完成工作。

- 能进入真实世界，不等于能稳定完成工作
- 有工具和数据可用，不等于有一套可持续运转的执行系统
...
[^8]: ["Improving Deep Agents with harness engineering"](https://blog.langchain.com/improving-deep-agents-with-harness-engineering/), LangChain Blog, Feb 17, 2026. 文中给出的具体结果是：在 Terminal Bench 2.0 上，同一模型 `gpt-5.2-codex` 仅通过调整 harness，就从 `52.8%` 提升到 `66.5%`。
