...
graph TB
    ...
    Agent --> Slack["Slack messages ❌<br/>Requires creating a Slack app and admin approval"]
    Agent --> Jira_t["Jira tickets ❌<br/>IT has disabled Personal Access Tokens"]
    Agent --> Cal["Google Calendar ❌<br/>OAuth app requires admin whitelist"]
    Agent --> Email["Email ❌<br/>Corporate IMAP is blocked by security policy"]
    
    Git --> OK["✅ Accessible"]
    Slack --> Blocked["❌ Inaccessible"]
    Jira_t --> Blocked
    Cal --> Blocked
    Email --> Blocked
...
graph TB
    subgraph "Wall 1: Platform Lockdown"
        W1["WhatsApp, Amazon, DoorDash, TikTok<br/>No open API<br/>Their business model depends on keeping it closed"]
    end
    
    subgraph "Wall 2: Organizational Control"
        W2["Slack, Jira, Google Workspace<br/>Platform has an API, but corporate IT won't approve it<br/>You can see the data, but your program can't read it"]
    end
